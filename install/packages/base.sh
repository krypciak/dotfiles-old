#!/bin/bash
artix_bare_install() {
    echo "$KERNEL $KERNEL-headers artix-keyring artix-mirrorlist autoconf automake base bison elogind-openrc fakeroot flex gcc groff iptables-nft libtool m4 make openntpd-openrc openrc pacman patch pkgconf texinfo util-linux which linux-firmware"
}
